...
\begin{itemize}
	...
	\item DAC - Discretionary Access Control %TODO Check DAC
\end{itemize}
...
\textbf{Introduction}  In the first chapter, it gives the purpose of CLup. It provided the definitions, acronyms and abbreviations in the document. And also describe the world phenomena and shared phenomena. \\


\textbf{Overall Description} In this chapter, We describe how the product works. In addition, we use the model of class diagrams and state diagrams to present the processing. And product functions gives the more details of functional and non-functional requirements. The list of domain assumptions and goals gives the support of the product.  The chapter ends with constrains to customer, manager and store back-end system, which are the basic for proceeding the product.
\\

\textbf{Specific Requirements} This chapter gives introduction to the system respect to external interfaces, functional requirements, scenarios and sequence diagrams. In use cases, it show the view from the click customer, brick customer and store manager. By using sequence diagram, illustrate the operations between each other. We also provide three small scenarios to explain more details and significant meaningful of CLup, corresponding to the view of click customer, brick customer and store manager.\\
...
The State diagram Fig.\ref{State Diagram 2} showed us how the manager monitors Store's On-Time data. The manager can view it on the Management System in computer, and he can know the number of customer inside store, and the customer queuing outside the store. He has the right to set the max people number inside the store to balance the capacity of store. In advance, if too many people waiting outside the store, he can also reschedule some customers' booking and send the notification to them. \\
...
\begin{figure}[H]  
	\centering 
	\includegraphics[scale=0.62]{State_diagram1}
...
\begin{figure}[H] 
	\centering
	\includegraphics[scale=0.43]{State_diagram2}
...
\begin{figure}[H] 
	\centering
	\includegraphics[scale=0.38]{State_diagram3}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Check on the notification from store manager when their booking is rescheduled.
	...
	\begin{itemize}
		...
		\item Received and schedule the click customers' booking. The scheduling must refer to the duration time of each customer and the categories of items that the customer intends to buy
		...
		\item Send a notification and update the E-Ticket to the customer when their booking is rescheduled.
		...
		\item Schedule or reschedule the queue order from the click customer's booking and the brick customer's retrieved Ticket.
...
\begin{itemize}
	...
	\item The Store Back-End System must mix the booking and brick customer's retrieved ticket reasonably to allow the click customers to enter the store near the booking time by avoiding making the brick customers wait too long.
...
\begin{itemize}
	...
	\item $D_4$ : If something unexpected happens, the store manager can adjust the maximum number of people in the store or reschedule the queue \& customer's booking reasonably.
...
For the \hyperref[Definitions]{Store Back-End System}, we buy an Amazon EC2 IaaS to implement this system. %TODO Delete the Iaas

\chapter{Specific Requirements} \label{C3:SpecificRequirements}

\section{External Interface Requirements}
CLup can be used by the customer who has the ability to use the smartphone. The system will collect data, analyze the data, and create the 'Ticket'.
Common users can interface themselves with CLup through a mobile application, and the store manager can access the system through a PC application.
Here is shown every kind of interface that the front-end offers or needs to interact with the users and the back-end.


\subsection{User Interfaces}
In this section there are mock-ups of the user interfaces for the mobile application used by customer.\\

\textbf{Login} In the Login page Fig.\ref{UI-Login} there are fields for filling username and password in order to access the personal page. In case that the customer don't have an account, there is also the option to register an account.\\

\textbf{Sign-up} In register page Fig.\ref{UI-Register}, users need to insert the username, password and confirmed password to register to the CLup.\\
...
\textbf{Category} In this page Fig.\ref{fig:ui-category}, there are lists of product categories in the Store, the product categories corresponding to the different zone in Store. Users can choose one or more categories which they want to buy during the shopping.\\


\textbf{Manager Dashboard} In the manager dashboard Fig.\ref{fig:ui-dashboard}, the Store Manager can see the number of Customers both inside and outside of the Store. He has the right to control the \textbf{Max} number of people inside the Store. If too many or too few people in the same zone of the Store, he can reset the Max number of customer inside the store or adjust the Queue.
And also if too many people waiting outside the Store, he can click the 'Reschedule booking' button, to reschedule the Click Customer's Booking who haven't departed yet. \\


\begin{figure}[H]
	\begin{minipage}[t]{0.5\linewidth}
		\centering
		\includegraphics[scale=0.5]{UI-Login}
		\caption{UI-Login}
		\label{UI-Login}
	\end{minipage}%
	\begin{minipage}[t]{0.5\linewidth}
		\centering
		\includegraphics[scale=0.5]{UI-Register}
		\caption{UI-Register}
		\label{UI-Register}
	\end{minipage}
\end{figure}
...
\begin{figure}[H]
	\begin{minipage}[t]{0.5\linewidth}
		\centering
		\includegraphics[scale=0.5]{UI-Home}
		\caption{UI-Home}
		\label{UI-Home}
	\end{minipage}%
	\begin{minipage}[t]{0.5\linewidth}
		\centering
		\includegraphics[scale=0.5]{UI-Booking}
		\caption{UI-Booking}
		\label{UI-Booking}
	\end{minipage}
\end{figure}
...
\begin{figure}[H] 
	\centering  
	\includegraphics[scale=0.5]{UI-Category}
	\caption{UI-Category}
	\centering
	\label{fig:ui-category}
\end{figure}

\begin{figure}[H] 
	\centering  
	\includegraphics[scale=0.4]{UI-Dashboard}
...
\begin{itemize}
	\item For \textbf{Click Customer}: the basic requirement is that a smartphone can access the Internet, and the GPS module is optional. If their smartphone is available, they can automatically input the address information by the GPS module. Otherwise, they have to input it manually.
	\item For \textbf{Store Manager}: It needs a simple pc can access our Back-End System through the Management System. Because we adopt the \hyperref[Reference documents]{Thin Client} strategy, the hardware requirements for PC configuration are very low.
	...
	\item For \textbf{Back-End System}: We adopted the Buy strategy. We will buy an Amazon EC2 \hyperref[Reference documents]{IaaS} to implement this system. So the server supplier Amazon will provide the hardware.
\end{itemize}
...
Our Click Customer's Mobile applications will run in Android or IOS System on their mobile devices.

And for the Store Manager's PC, It needs an execution environment that can execute the Management System programs.



\subsection{Communication Interfaces}

We have some kinds of devices that have to communicate with each other, Click Customer's mobile application, Store Manage's PC, the back-end system, the \hyperref[Definitions]{Digital Counterpart}, and the Tickets Hand-Out Machine.
The Mobile Phone communicates with the Back-End system via the internet with TCP protocol in the transport layer.

For our hardware devices: Digital Counterpart, Tickets Hand-Out Machine, and Store Manager's PC.
They will communicate with the Back-End System via a physical network cable work in Ethernet.
A design like this is because they are fixed facilities, they do not need "Mobility", but need high availability and stability.
...
\begin{figure}
	\includegraphics[angle=270,scale=0.2]{usecase_diagram}
	\centering
...
From the customer's perspective Fig.\ref{fig:sequence-diagram-customer}, two kinds of customers are independent and do not interfere. The queue schedule function calls the queue number via the Digital Counterpart, and the customer has to view or listen well their queue number. When their number is called, they scan their QRCode.

From the manager's perspective Fig.\ref{fig:sequence-diagram-manager}, their four operations are independent and do not interfere with each other.


\begin{figure}
	\includegraphics[scale=0.22]{sequence_diagram_customer}
	\centering
	\caption{Customers' Sequence Diagram}
	\label{fig:sequence-diagram-customer}
\end{figure}

\begin{figure}
	\includegraphics[scale=0.18]{sequence_diagram_manager}
	\centering
	\caption{Manager's Sequence Diagram}
	\label{fig:sequence-diagram-manager}
\end{figure}
...
For the Click Customer's Application, it has to can run well in Android and IOS systems.
And for our Management System, it can run on a pc with a suitable execution environment. \\
...
\lstinputlisting[linerange={1-165}]{./AlloyCode/CLupAlloy.als}
...
\lstinputlisting[linerange={168-176}]{./AlloyCode/CLupAlloy.als}
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.8]{BrickCustomerRunAlloy}
\end{figure}


\begin{figure}
	\centering
	\includegraphics[ angle = 270, scale=0.45]{BrickCustomerAlloy}
	\caption{The Alloy world of the Brick Customer's perspective}
	\centering
	\label{BrickCustomerAlloy}
\end{figure}

\vspace{10mm}

From the Click Customer's perspective Fig.\ref{CrickCustomerAlloy}, they have a unique Username, and some of them may do not hold a Booking, for example, a new User. To the Click Customer with a valid Booking, his Valid Booking's Valid Ticket may still not out. However, when they are in the QueueSchedule, they must hold a Valid Booking with a Valid Ticket. This constraint has already been declared in fact part.
Now we ran a case with some different situations.
\lstinputlisting[linerange={180-193}]{./AlloyCode/CLupAlloy.als}
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.3]{ClickCustomerRunAlloy}
\end{figure}


\begin{figure}
	\centering
	\includegraphics[ angle = 270, scale=0.45]{ClickCustomerAlloy}
	\caption{The Alloy world of the Click Customer's perspective}
	\centering
	\label{CrickCustomerAlloy}
\end{figure}
...
Finally, we are trying to give a "General World" of our Alloy model, shown in Fig.\ref{GeneralWorldAlloy}.\\

\lstinputlisting[linerange={196-207}]{./AlloyCode/CLupAlloy.als}
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.35]{GeneralWorldRunAlloy}
\end{figure}


\begin{figure}
	\centering
	\includegraphics[ angle = 270, scale=0.4]{GeneralWorldAlloy}
	\caption{The Alloy General World}
	\centering
	\label{GeneralWorldAlloy}
\end{figure}
...
\begin{itemize}
	...
	\begin{center}
		...
		\begin{tabular}{ |c|c|c| } 
			...
			\hline
			2020/12/10 & Wrote the overview & 2h \\
			\hline
			...
			\hline
			2020/12/22 & Modified UI and RASD revision& 3h \\
			\hline
